		...
		<table id="tablebody" width="100%"
			...
			<thead style="padding-buttom: 30px;">
				<tr class="line head">
					...
					<th style="font-size: 18px; text-align: center; width: 12%;">Delay Time<!-- <br>(Delay Time)  -->
					</th>
	...
	<script type="text/javascript">
		...
		$(document).ready(function(){
			<c:forEach items="${board}" var="vo">
				...
				if(state==="배송중"){			//DB저장된 한글 영어로 바꾸기
					if(result){
						...
						if(delayedDay==0 && delayedHour==0){
							var delayed = delayedMin+'min';
						}
						else if(delayedDay==0){
							var delayed = delayedHour+'hour  '+delayedMin+'min';
						}
						else{
							var delayed = delayedDay+'day  '+delayedHour+'hour  '+delayedMin+'min';
						}
				...
				}else if(state === "배송완료"){		//DB저장된 한글 영어로 바꾸기
